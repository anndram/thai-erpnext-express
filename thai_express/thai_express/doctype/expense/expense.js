...
frappe.ui.form.on('Expense', {
    ...
    refresh: function(frm) {
        if (!frm.X.is_requested && !frm.is_new()) frm.trigger('add_toolbar_button');
    },
    ...
    set_account_data: function(frm) {
        if (frm.X.is_requested || !frm.is_dirty()) return;
        var company = frm.doc.company,
        item = frm.doc.expense_item;
        if (!company || !item) {
            frm.set_value('expense_account', '');
            frm.set_value('currency', '');
            frm.X.expense_cost = frm.X.expense_qty = null;
            return;
        }
        frappe.E.call(
            ...
            function(ret) {
                ...
                if (flt(ret.cost) > 0) {
                    ...
                    frm.X.expense_cost = {min: flt(ret.min_cost), max: flt(ret.max_cost)};
                }
                if (flt(ret.qty) > 0) {
                    frm.set_value('qty', ret.qty);
                    frm.toggle_enable('qty', 0);
                } else if (flt(ret.min_qty) > 0 || flt(ret.max_qty) > 0) {
                    frm.X.expense_qty = {min: flt(ret.min_qty), max: flt(ret.max_qty)};
                }
            }
        );
    },
    required_by: function(frm) {
        if (frm.X.is_super || !frm.doc.required_by) return;
        let date = moment(),
    ...
    cost: function(frm) {
        if (frm.X.is_requested) return;
        let cost = flt(frm.doc.cost),
        limit = frm.X.expense_cost,
    ...
    is_paid: function(frm) {
        if (!cint(frm.doc.is_paid)) {
            ...
            frm.X.with_expense_claim && frm.set_value('expense_claim', '');
        }
        if (frm.X.with_expense_claim) {
    ...
    paid_by: function(frm) {
        if (!frm.doc.paid_by && frm.X.with_expense_claim) {
    ...
    after_save: function(frm) {
        if (!frm.X.is_requested && frm.X.del_files.length) {
            frappe.E.call(
                'delete_attach_files',
                {
                    doctype: frm.doctype,
